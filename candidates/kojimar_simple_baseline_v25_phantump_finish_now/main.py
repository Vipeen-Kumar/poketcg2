...
class LucarioPolicy:
    ...
    def _opponent_is_phantump_control(self) -> bool:
        return self._opponent_has({C.HOPS_PHANTUMP, C.HOPS_TREVENANT})

    def _should_finish_phantump_ko_now(self) -> bool:
        return self._opponent_is_phantump_control() and plan.target >= 0 and plan.remain_hp <= 0
    ...
    def _score_option(self, option) -> float:
        ...
        if option.type == OptionType.ATTACK:
            if self._should_finish_phantump_ko_now():
                return 40000 if (option.attackId == MEGA_BRAVE) == (plan.attack_index == 1) else 39000
            if (
                self._opponent_is_crustle_wall()
                and self.me.active
